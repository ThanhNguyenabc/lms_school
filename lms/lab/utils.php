...
function SQL_Query($query, $connection, $cache = null)
{ 
 // DETERMINE EXECUTION MODE
 $redis = null;
 if($cache) $redis  = Open_Connect_Redis();
 ...
 if(str_starts_with($query, "INSERT") || str_starts_with($query, "DELETE"))
 {
  ...
  if($redis)
  {
    ...
    {
      $redis->unlink($cache["key"]);
      $keys = $redis->keys($cache["key"] . "*");
      $redis->unlink($keys);
    }
    else
    { 
      $keysSearch = $redis->keys($cache["key"] . ":search*");
      $redis->unlink($keysSearch);
    }
    $redis->close();
  }
 }
 else
 {
  ...
  if($redis && str_starts_with($query, "SELECT"))
  {
 ...
 switch($mode)
 {
	...
	if($redis)
  {
    ...
    {
      $ex = (int)$cache["timetolive"] == 0 ? 86400 : (int)$cache["timetolive"];
      $redis->set($cache["key"],serialize($data),['nx', 'ex' => $ex]);     
    } 
    else
    {
      //REMOVE KEY SEARCH OR ALL KEY 
      $redis->unlink($cache["key"]);
      $keys = explode(":",$cache["key"]);
      if(count($keys) > 1)
      {
        $keysSearch = $redis->keys($keys[0]. ":search*");
      }
      else{
        $keysSearch = $redis->keys($keys[0] . "*");
      }
      $redis->unlink($keysSearch);
    } 
    $redis->close();
...
function Open_Connect_Redis()
{
  try {
    if(isset($_SESSION["config"]))
    {
      ...
      $username   = $config["system"]["redis"]["username"];
    }
    else
    {
      $config = parse_ini_file("partners/default/system.cfg",true);
